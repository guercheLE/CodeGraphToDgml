...
internal sealed class DgmlDocumentService
{
    ...
    {
        var directory = await TempDirectoryHelper.GetOutputDirectoryAsync(_package).ConfigureAwait(true);
        var filePath = Path.Combine(directory, $"CodeGraph-{DateTime.Now:yyyyMMdd-HHmmssfff}.dgml");
    ...
    /// <summary>
    /// Opens a document via VsShellUtilities, falling back to DTE when the VS shell
    /// throws NullReferenceException for external/temp files.
    /// </summary>
    internal static IVsWindowFrame OpenDocumentSafe(IServiceProvider serviceProvider, string fullPath)
    {
...
internal sealed class DgmlDocumentSession
{
    ...
    public void ReplaceAllText(string newText)
    {
        ...
        if (_textLines is null)
        {
            ...
            _windowFrame = DgmlDocumentService.OpenDocumentSafe(_package, FullPath);
