...
- New command "Traverse Down to Sequence Diagram (with Control Flow)": calls are wrapped in Mermaid `alt`/`opt`/`loop`/`break` fragments derived from the enclosing `if`/`switch`/loop/`catch` statements, interface and virtual dispatch renders as an `alt` per implementation, and split parts re-open fragments with a `(cont.)` marker. Works for C# and Visual Basic. New option "Max condition label length". The existing sequence command is unchanged.
